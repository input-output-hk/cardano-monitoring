    ...
    modules = [
      {
        ...
        data.aws_s3_bucket = lib.listToAttrs (map (bucket: {
            name = bucket;
            value = {inherit bucket;};
          })
          buckets);

        resource.aws_s3_bucket_policy = lib.listToAttrs (map (bucket: {
            name = "allow_${bucket}";
            value = {
        ...
        data.aws_iam_policy_document = lib.listToAttrs (map (bucket: {
            name = "allow_${bucket}";
            value = {
              statement = {
                principals = {
        ...
        resource.aws_route53_record = mapNodes (
          nodeName: _: {
        ...
        resource.tls_private_key.bootstrap.algorithm = "ED25519";

        resource.aws_key_pair = mapRegions ({
          count,
          region,
        }: {
          "bootstrap_${region}" = {
        ...
        resource.aws_security_group = let
